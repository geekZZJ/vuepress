...
`慢启动算法`作用在拥塞产生之前：对于刚刚加入网络的连接，要一点一点的提速，不要妄图一步到位。如下：
...
![慢启动算法](/network/16.png "慢启动算法")

### 拥塞避免算法

当`cwnd >= ssthresh`（通常`ssthresh = 65535`）时，就会进入拥塞避免算法：缓慢增长，小心翼翼的找到最优值。如下：

1. 每收到一个`ACK`，`cwnd = cwnd + 1/cwnd`
2. 每经过一个`RTT`，`cwnd++`，线性增长

慢启动算法主要呈指数增长，粗犷型，速度快（“慢”是相对于一步到位而言的）；而拥塞避免算法主要呈线性增长，精细型，速度慢，但更容易在不导致拥塞的情况下，找到网络环境的`cwnd`最优值

### 拥塞发生时的算法

如果已经发生拥塞，则需要采取策略减小`cwnd`。那么，`TCP`如何判断当前网络拥塞了呢？很简单，如果发送方发现有`Seq`发送失败（表现为“丢包”），就认为网络拥塞了
...
如果触发了快速重传，即发送方收到至少 3 次相同的`ACK`，那么 TCP 认为网络情况不那么糟，也就没必要提心吊胆的，可以适当大胆的恢复。为此设计快速恢复算法，下面介绍快速恢复的实现
...
![快速恢复](/network/17.png "快速恢复")

## CDN 原理

CDN (全称 Content Delivery Network)，即内容分发网络

`CDN`就是根据用户位置分配最近的资源。用户在上网的时候不用直接访问源站，而是访问离他“最近的”一个`CDN`节点，术语叫**边缘节点**，其实就是缓存了源站内容的代理服务器。如下图：

![CDN](/network/cdn.png "CDN")
